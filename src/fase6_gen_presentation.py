...
def create_presentation():
    ...
    eq_text = (
        ...
        "G_poa = G_b · R_beam + G_d · [(1 + cos(β)) / 2] +\n"
        "        G · ρ · [(1 - cos(β)) / 2]\n\n"
        "2. Irradiancia Absorbida por Celdas (S):\n"
        "S / S_ref = (G_b/G_ref) · R_beam · K_τα,b +\n"
        "            (G_d/G_ref) · K_τα,d · [(1 + cos(β)) / 2] +\n"
        "            (G/G_ref) · ρ · K_τα,g · [(1 - cos(β)) / 2]\n\n"
        "Donde S_ref = G_ref = 1000 W/m² (SRC)."
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), eq_text, size=15, color=ACCENT_GOLD)
    ...
    add_text(slide, MARGIN + COLUMN_WIDTH + GAP + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), glossary, size=14)
    ...
    iam_text = (
        ...
        "K_τα(θ) = τ(θ) / τ(0)\n\n"
        "τ(θ) = exp(-K·L / cos(θ_r)) · [1 - 0.5 · (\n"
        "       sin²(θ_r − θ)/sin²(θ_r + θ) + \n"
        "       tan²(θ_r − θ)/tan²(θ_r + θ) )]\n\n"
        "θ_r = arcsin(sin(θ) / n)\n\n"
        "Glosario:\n"
        "• θ: Ángulo de incidencia solar respecto a la normal del panel.\n"
        "• n = 1.526 (Índice de refracción del vidrio templado).\n"
        "• K = 4 m⁻¹ (Coeficiente de absorción del vidrio).\n"
        "• L = 2 mm (Espesor típico del vidrio del panel)."
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), iam_text, size=13, color=ACCENT_GOLD)
    ...
    am_text = (
        ...
        "M / M_ref = a_0 + a_1 · AM + a_2 · AM² + a_3 · AM³ + a_4 · AM⁴\n\n"
        "AM = 1 / [cos(θ_z) + 0.5057 · (96.08 − θ_z)⁻¹·⁶³⁴]\n\n"
        "Glosario:\n"
        "• AM: Masa de aire absoluta (corregida por presión atmosférica local).\n"
        "• θ_z: Ángulo cenital del sol.\n"
        "• a_0 a a_4: Coeficientes empíricos espectrales según el tipo de semiconductor (establecidos por De Soto).\n"
        "• M_ref: Transmitancia del espectro solar en STC (AM 1.5g)."
    )
    add_text(slide, MARGIN + COLUMN_WIDTH + GAP + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), am_text, size=13)
    ...
    thermal_eq = (
        "La temperatura interna de la celda (Tc) depende de la irradiancia, temperatura ambiente y viento:\n\n"
        "T_c = G_poa · exp(a + b · v_w) + T_a + (G_poa / 1000) · ΔT\n\n"
        "Glosario de Términos:\n"
        "• T_c / T_a: Temperatura de celda y ambiental (°C).\n"
        "• G_poa: Irradiancia en el plano del panel (W/m²).\n"
        "• v_w: Velocidad del viento (asumida 1 m/s constante).\n"
        "• a, b, ΔT: Parámetros empíricos según encapsulado físico."
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), thermal_eq, size=15, color=ACCENT_GOLD)
    ...
    sdm_eq = (
        "El módulo se modela como un circuito de un diodo y resistencias de pérdidas:\n\n"
        "I = I_L − I_0 · [ exp((V + I·R_s) / a) − 1 ] − (V + I·R_s) / R_sh\n\n"
        "El factor de idealidad modificado (a) se define por la física del semiconductor:\n\n"
        "a = (N_s · n_I · k · T_c) / q"
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), sdm_eq, size=15, color=ACCENT_GOLD)
    ...
    sdm_glossary = (
        ...
        "• I_L: Corriente fotogenerada por luz (A) (light current).\n"
        "• I_0: Corriente de saturación inversa del diodo (A).\n"
        "• R_s: Resistencia serie (pérdidas por conducción) (Ω).\n"
        "• R_sh: Resistencia paralelo o shunt (corrientes de fuga) (Ω).\n"
        "• N_s: Número de celdas conectadas en serie (mSi: 36, HIT: 72).\n"
        "• n_I: Factor de idealidad del diodo (adimensional).\n"
        "• k: Constante de Boltzmann (1.3806e-23 J/K).\n"
        "• q: Carga del electrón (1.6022e-19 C).\n"
        "• T_c: Temperatura de la celda (K)."
    )
    add_text(slide, MARGIN + COLUMN_WIDTH + GAP + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), sdm_glossary, size=13)
    ...
    src_eq = (
        "Se extraen aref, IL,ref, I0,ref, Rs,ref, Rsh,ref en STC (1000 W/m², 25°C):\n\n"
        "1. Cortocircuito (Isc): V = 0, I = Isc,ref\n"
        "2. Circuito Abierto (Voc): I = 0, V = Voc,ref\n"
        "3. Máxima Potencia (MPP): I = Imp,ref, V = Vmp,ref\n"
        "4. Pendiente en MPP (dP/dV = 0):\n"
        "   dI/dV|_mp = -Imp,ref / Vmp,ref\n"
        "5. Coeficiente térmico de Voc: β_Voc = ∂Voc/∂Tc"
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), src_eq, size=14, color=ACCENT_GOLD)
    ...
    deriv_eq = (
        "Para acoplar Rs y el factor de idealidad, se implementa la derivada analítica obtenida del circuito:\n\n"
        "dI/dV|_mp = - [ A + B ] / [ 1 + Rs · A + Rs · B ]\n\n"
        "Donde:\n"
        "  A = (I_0 / a) · exp((V_mp + I_mp · Rs) / a)\n"
        "  B = 1 / R_sh\n\n"
        "La optimización se realiza mediante ajuste de mínimos cuadrados con bounds físicos (Rs > 0, n_I ∈ [1, 2])."
    )
    add_text(slide, MARGIN + COLUMN_WIDTH + GAP + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), deriv_eq, size=13)
    ...
    scale_eq = (
        "1. Factor de Idealidad: a / a_ref = T_c / T_ref\n\n"
        "2. Corriente de Saturación Inversa (Io):\n"
        "Io / Io,ref = (T_c/T_ref)³ · exp[(E_g,ref/(k·T_ref)) − (E_g/(k·T_c))]\n\n"
        "3. Energía de Bandgap (Eg) del Silicio:\n"
        "E_g / E_g,ref = 1 − 0.0002677 · (T_c − T_ref)\n"
        "(E_g,ref = 1.121 eV para Si cristalino a 25°C)\n\n"
        "4. Corriente Fotogenerada (IL):\n"
        "I_L = (S/S_ref) · (M/M_ref) · [I_L,ref + α_Isc · (T_c − T_ref)]\n\n"
        "5. Resistencia Shunt: R_sh = R_sh,ref · (S_ref / S)"
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), scale_eq, size=13, color=ACCENT_GOLD)
    ...
    pr_eq = (
        ...
        "Glosario de Términos:\n"
        "• P_mp,SDM: Potencia en el MPP simulada minuciosamente hora a hora usando el modelo De Soto (W).\n"
        "• P_STC: Potencia nominal del panel a STC (m-Si: 46.68 W | HIT: 217.52 W).\n"
        "• G_poa: Irradiancia calculada en el plano del panel (W/m²).\n"
        "• 1000: Irradiancia de referencia a STC (W/m²)."
    )
    add_text(slide, MARGIN + Inches(0.2), Inches(1.8), COLUMN_WIDTH - Inches(0.4), Inches(4.5), pr_eq, size=15, color=ACCENT_GOLD)
